...
export const CheckoutFlow: React.FC = () => {
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="bg-white dark:bg-navy-900/40 p-8 rounded-3xl border border-silver-100 dark:border-navy-800 shadow-sm">
              ...
              <div className="space-y-3">
                {UPSELLS.map(u => (
                  <label key={u.id} className={`flex items-center justify-between p-4 rounded-xl border cursor-pointer transition-colors ${state.upsells.includes(u.id) ? 'border-navy-900 dark:border-white bg-navy-50/30 dark:bg-navy-800/30' : 'border-silver-100 dark:border-navy-800'}`}>
                    <div className="flex items-center gap-4">
                      <input 
                        type="checkbox" 
                        checked={state.upsells.includes(u.id)}
                        onChange={() => handleUpsellToggle(u.id)}
                        className="w-5 h-5 accent-navy-900" 
                      />
                      <div>
                        <p className="font-bold text-sm dark:text-white">{u.name}</p>
                        <p className="text-[10px] text-navy-400 dark:text-navy-300">{u.description}</p>
                      </div>
                    </div>
                    <span className="font-bold text-sm dark:text-white">+${u.price}</span>
                  </label>
                ))}
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen pt-24 pb-48 px-6 bg-transparent transition-colors">
