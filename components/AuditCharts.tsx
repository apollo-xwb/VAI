...
export function CallVolumeBarChart() {
  const max = 46;
  const barMaxPx = 200;
  const bars = [
    { label: 'Estimated calls', value: 42, grad: 'url(#cv-gold)', text: '38–46 / wk' },
    { label: 'Answered calls', value: 15, grad: 'url(#cv-silver)', text: '13–17 / wk' },
    { label: 'Missed calls', value: 27, grad: 'url(#cv-rose)', text: '24–29 / wk' },
  ];
  return (
...
export function RevenueLeakLineChart() {
  const points = [18000, 38000, 58000, 78000, 102000, 128000];
  const w = 400;
  const h = 180;
  const pad = 16;
  const maxY = 140000;
...
export function MissedCallDonutChart() {
  const segments = [
    { pct: 52, gradId: 'donut-gold', label: 'Pricing inquiries' },
    { pct: 31, gradId: 'donut-silver', label: 'Custom / engagement' },
    { pct: 17, gradId: 'donut-steel', label: 'Appointments' },
  ];
  let acc = 0;
  const r = 52;
  const cx = 60;
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-zinc-900/90 via-red-950/25 to-black p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">
        ...
        <p className="mt-3 text-2xl font-semibold text-slate-100">34% answered</p>
        <p className="mt-2 text-lg font-medium text-rose-200/95">~$21,400 lost / month</p>
        <p className="mt-3 text-xs text-slate-400">Based on typical luxury retail answer rates and your call volume</p>
      </div>
      <div className="rounded-3xl border border-amber-400/25 bg-gradient-to-br from-zinc-950 via-amber-950/30 to-black p-6 shadow-[inset_0_1px_0_rgba(212,175,55,0.18)]">
        ...
        <p className="mt-3 text-2xl font-semibold tracking-tight text-slate-100">99% answered</p>
        <p className="mt-2 text-lg font-medium text-amber-200/95">$0 lost to missed calls</p>
    ...
    <div className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-zinc-950 to-black p-6 md:p-8">
      ...
      <div className="mt-4 rounded-2xl border border-white/10 bg-black/50 p-5 shadow-inner">
        ...
        <p className="mt-4 text-sm italic text-slate-300">
          &ldquo;Beautiful pieces, but I called twice and never got through.&rdquo;
        </p>
        <p className="mt-2 text-sm italic text-slate-300">
          &ldquo;Voicemail every time — ended up buying elsewhere.&rdquo;
        </p>
...
export function TrafficPieChart() {
  const slices = [
    { label: 'Google Search', pct: 68, id: 'pie-g' },
    { label: 'Website', pct: 22, id: 'pie-w' },
    { label: 'Direct / Other', pct: 10, id: 'pie-d' },
  ];
  let acc = 0;
  const r = 45;
  const cx = 50;
